...
class InvoiceService {
  ...
  async generateOrderInvoice(order, customer, seller) {
    try {
      console.log(`📄 Auto-generating invoice for order: ${order.orderNumber}`);
      ...
      console.log(`✅ Invoice generated: ${invoice.invoiceNumber} (IPFS: ${invoice.ipfsHash})`);
      return invoice;
    } catch (error) {
      console.error("❌ Failed to generate order invoice:", error);
      throw error;
    }
  }
  ...
  async generateVendorRequestInvoice(vendorRequest, vendor, supplier) {
    try {
      console.log(`📄 Auto-generating purchase invoice for request: ${vendorRequest.requestNumber}`);
      console.log(`📋 Vendor Request Details:`, {
      ...
      console.log(`✅ Purchase invoice generated: ${invoice.invoiceNumber}`);
      return invoice;
    } catch (error) {
      console.error("❌ Failed to generate vendor request invoice:", error);
      throw error;
    }
  }
  ...
  async generateInventoryReceipt(vendorInventory, vendor, supplier) {
    try {
      console.log(`📄 Auto-generating receipt for inventory delivery`);
      ...
      console.log(`✅ Receipt generated: ${invoice.invoiceNumber}`);
      return invoice;
    } catch (error) {
      console.error("❌ Failed to generate inventory receipt:", error);
      throw error;
    }
  }
  ...
  async _createAndUploadInvoice(invoiceData) {
    ...
    console.log(`📝 Invoice created in MongoDB: ${invoice.invoiceNumber}`);
    ...
    try {
      ...
      console.log(`🔄 Uploading PDF to IPFS: ${invoice.invoiceNumber}.pdf`);
      ...
      if (ipfsResult.success) {
        ...
        console.log(`✅ Invoice uploaded to IPFS: ${ipfsResult.ipfsHash}`);
        ...
        try {
          ...
          console.log(`✅ Invoice recorded on blockchain: ${blockchainResult.txId}`);
        } catch (blockchainError) {
          console.error("⚠️ Blockchain recording failed (non-critical):", blockchainError.message);
        } finally {
          await fabricService.disconnect();
        }
      } else {
        console.warn("⚠️ IPFS upload failed (non-critical)");
      }
  ...
  async downloadInvoice(invoiceId) {
    ...
    if (invoice.ipfsHash) {
      try {
        console.log(`📥 Retrieving invoice from IPFS: ${invoice.ipfsHash}`);
        const ipfsResult = await ipfsService.getFile(invoice.ipfsHash);

        if (ipfsResult.success && ipfsResult.data) {
          console.log(`✅ Invoice retrieved from IPFS: ${invoice.invoiceNumber}`);
          // IPFS returns the data as a buffer or string
          return Buffer.isBuffer(ipfsResult.data)
            ? ipfsResult.data
            : Buffer.from(ipfsResult.data);
        } else {
          console.warn(`⚠️ IPFS retrieval failed, regenerating PDF: ${ipfsResult.error}`);
        }
      } catch (ipfsError) {
        console.warn(`⚠️ IPFS retrieval error, regenerating PDF:`, ipfsError.message);
      }
    }

    // Fallback: Regenerate PDF if IPFS retrieval failed or no hash exists
    console.log(`🔄 Regenerating invoice PDF: ${invoice.invoiceNumber}`);
