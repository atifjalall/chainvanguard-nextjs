...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        "❌ GET /api/vendor/customers/:customerId/orders error:",
        error
      );
      res.status(500).json({
        success: false,
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const customer =
        await User.findById(customerId).select("name email phone");
      ...
      console.error(
        "❌ GET /api/vendor/customers/:customerId/contact error:",
        error
      );
      res.status(500).json({
        success: false,
