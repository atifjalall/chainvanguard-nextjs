...
export default function EditProductPage() {
  ...
  const [removeImages, setRemoveImages] = useState<string[]>([]); // Track images to remove
  ...
  const removeImage = (index: number) => {
    ...
    // If it's an existing image, add to removeImages
    if (
      imageToRemove &&
      ...
      setRemoveImages((prev) => [...prev, imageToRemove]); // Assuming imageToRemove is the publicId or URL to remove
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const updateData = {
        ...
        apparelDetails: formData.apparelDetails,
      ...
      try {
        ...
        if (response.success) {
          ...
          setFormData({
            ...
            productType: product.productType || "Casual",
            brand: product.brand || "",
            sku: product.sku || "",
            apparelDetails: {
              size: product.apparelDetails?.size || "",
              fit: product.apparelDetails?.fit || "Regular Fit",
              color: product.apparelDetails?.color || "",
              pattern: product.apparelDetails?.pattern || "Solid",
              ...
              careInstructions:
                product.apparelDetails?.careInstructions ||
                "Machine wash cold, tumble dry low",
              neckline: product.apparelDetails?.neckline || "Crew Neck",
              sleeveLength:
                product.apparelDetails?.sleeveLength || "Short Sleeve",
            },
            price: product.price?.toString() || "",
            costPrice: product.costPrice?.toString() || "",
            quantity: product.quantity?.toString() || "",
            minStockLevel: product.minStockLevel?.toString() || "20",
            ...
            imagePreviews: product.images?.map((img) => img.url) || [], // Existing images
          });
        }
      } catch (error) {
